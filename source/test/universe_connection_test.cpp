#include "StarUniverseConnection.hpp"
#include "StarTcp.hpp"
#include "StarTime.hpp"

#include "gtest/gtest.h"

using namespace Star;

unsigned const PacketCount = 20;
uint16_t const ServerPort = 55555;

unsigned const NumLocalASyncConnections = 5;
unsigned const NumRemoteASyncConnections = 5;
unsigned const ASyncSleepMillis = 5;

unsigned const NumLocalSyncConnections = 5;
unsigned const NumRemoteSyncConnections = 5;
unsigned const SyncWaitMillis = 10000;

template <typename Predicate>
bool waitUntil(Predicate predicate, unsigned timeoutMillis = 2000) {
  auto timer = Timer::withMilliseconds(timeoutMillis);
  while (!timer.timeUp()) {
    if (predicate())
      return true;
    Thread::sleep(1);
  }
  return predicate();
}

size_t totalOwnedConnections(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  size_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.ownedConnections;
  return total;
}

uint64_t totalConnectionScans(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.connectionScans;
  return total;
}

uint64_t totalPacketsProcessed(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.packetsProcessed;
  return total;
}

uint64_t totalQueuedSendBatches(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.queuedSendBatches;
  return total;
}

uint64_t totalEagerSendBatches(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.eagerSendBatches;
  return total;
}

uint64_t totalEagerSendPackets(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.eagerSendPackets;
  return total;
}

uint64_t totalWorkerSendBatches(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.workerSendBatches;
  return total;
}

uint64_t totalWorkerSendPackets(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.workerSendPackets;
  return total;
}

uint64_t totalWakeups(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.wakeups;
  return total;
}

uint64_t totalIdleTimedWaits(List<UniverseConnectionServer::NetworkWorkerStats> const& stats) {
  uint64_t total = 0;
  for (auto const& workerStats : stats)
    total += workerStats.idleTimedWaits;
  return total;
}

class ASyncClientThread : public Thread {
public:
  ASyncClientThread(UniverseConnection conn)
    : Thread("UniverseConnectionTestClientThread"), m_connection(std::move(conn)) {
    start();
  }

  virtual void run() {
    try {
      unsigned read = 0;
      unsigned written = 0;
      while (read < PacketCount || written < PacketCount) {
        m_connection.receive();
        if (read < PacketCount) {
          if (auto packet = m_connection.pullSingle()) {
            EXPECT_TRUE(convert<ProtocolRequestPacket>(packet)->requestProtocolVersion == read);
            ++read;
          }
        }

        if (written < PacketCount) {
          m_connection.push({make_shared<ProtocolRequestPacket>(written)});
          ++written;
        }
        m_connection.send();

        Thread::sleep(ASyncSleepMillis);

        if (!m_connection.isOpen())
          break;
      }

      EXPECT_EQ(PacketCount, read);
      EXPECT_EQ(PacketCount, written);
      m_connection.close();
      EXPECT_TRUE(m_connection.pull().empty());
    } catch (std::exception const& e) {
      ADD_FAILURE() << "Exception: " << outputException(e, true);
    } catch (...) {
      ADD_FAILURE();
    }
  }

private:
  UniverseConnection m_connection;
};

class SyncClientThread : public Thread {
public:
  SyncClientThread(UniverseConnection conn)
    : Thread("UniverseConnectionTestClientThread"), m_connection(std::move(conn)) {
    start();
  }

  virtual void run() {
    try {
      for (unsigned i = 0; i < PacketCount; ++i) {
        m_connection.pushSingle(make_shared<ProtocolRequestPacket>(i));
        EXPECT_TRUE(m_connection.sendAll(SyncWaitMillis));
        EXPECT_TRUE(m_connection.receiveAny(SyncWaitMillis));
        EXPECT_EQ(convert<ProtocolRequestPacket>(m_connection.pullSingle())->requestProtocolVersion, i);

        if (!m_connection.isOpen())
          break;
      }

      m_connection.close();
      EXPECT_TRUE(m_connection.pull().empty());
    } catch (std::exception const& e) {
      ADD_FAILURE() << "Exception: " << outputException(e, true);
    } catch (...) {
      ADD_FAILURE();
    }
  }

private:
  UniverseConnection m_connection;
};

TEST(UniverseConnections, All) {
  UniverseConnectionServer server([](UniverseConnectionServer* server, ConnectionId clientId, List<PacketPtr> packets) {
      server->sendPackets(clientId, packets);
    });

  ConnectionId clientId = ServerConnectionId;
  TcpServer tcpServer(HostAddressWithPort(HostAddress::localhost(), ServerPort));
  tcpServer.setAcceptCallback([&server, &clientId](TcpSocketPtr socket) {
      socket->setNonBlocking(true);
      auto conn = UniverseConnection(TcpPacketSocket::open(std::move(socket)));
      server.addConnection(++clientId, std::move(conn));
    });

  LinkedList<ASyncClientThread> localASyncClients;
  for (unsigned i = 0; i < NumLocalASyncConnections; ++i) {
    auto pair = LocalPacketSocket::openPair();
    server.addConnection(++clientId, UniverseConnection(std::move(pair.first)));
    localASyncClients.emplaceAppend(UniverseConnection(std::move(pair.second)));
  }

  LinkedList<SyncClientThread> localSyncClients;
  for (unsigned i = 0; i < NumLocalSyncConnections; ++i) {
    auto pair = LocalPacketSocket::openPair();
    server.addConnection(++clientId, UniverseConnection(std::move(pair.first)));
    localSyncClients.emplaceAppend(UniverseConnection(std::move(pair.second)));
  }

  LinkedList<ASyncClientThread> remoteASyncClients;
  for (unsigned i = 0; i < NumRemoteASyncConnections; ++i) {
    auto socket = TcpSocket::connectTo({HostAddress::localhost(), ServerPort});
    socket->setNonBlocking(true);
    remoteASyncClients.emplaceAppend(UniverseConnection(TcpPacketSocket::open(std::move(socket))));
  }

  LinkedList<SyncClientThread> remoteSyncClients;
  for (unsigned i = 0; i < NumRemoteSyncConnections; ++i) {
    auto socket = TcpSocket::connectTo({HostAddress::localhost(), ServerPort});
    socket->setNonBlocking(true);
    remoteSyncClients.emplaceAppend(UniverseConnection(TcpPacketSocket::open(std::move(socket))));
  }

  for (auto& c : localASyncClients)
    c.join();

  for (auto& c : remoteASyncClients)
    c.join();

  for (auto& c : localSyncClients)
    c.join();

  for (auto& c : remoteSyncClients)
    c.join();

  server.removeAllConnections();
}

TEST(UniverseConnectionServer, WorkerOwnershipStats) {
  UniverseConnectionServer server([](UniverseConnectionServer*, ConnectionId, List<PacketPtr>) {}, 2);

  List<UniverseConnection> clients;
  for (ConnectionId clientId = 1; clientId <= 4; ++clientId) {
    auto pair = LocalPacketSocket::openPair();
    server.addConnection(clientId, UniverseConnection(std::move(pair.first)));
    clients.append(UniverseConnection(std::move(pair.second)));
  }

  auto stats = server.workerStats();
  ASSERT_EQ(2, stats.size());
  EXPECT_EQ(4, totalOwnedConnections(stats));
  EXPECT_EQ(2, stats[0].ownedConnections);
  EXPECT_EQ(2, stats[1].ownedConnections);

  auto removed = server.removeConnection(2);
  EXPECT_FALSE(server.hasConnection(2));

  stats = server.workerStats();
  EXPECT_EQ(3, totalOwnedConnections(stats));
  EXPECT_EQ(1, stats[0].ownedConnections);
  EXPECT_EQ(2, stats[1].ownedConnections);
}

TEST(UniverseConnectionServer, SendPacketsWakesIdleWorker) {
  UniverseConnectionServer server([](UniverseConnectionServer*, ConnectionId, List<PacketPtr>) {}, 1);

  auto pair = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pair.first)));
  UniverseConnection client(std::move(pair.second));

  auto beforeWakeups = server.workerStats()[0].wakeups;
  server.sendPackets(1, {make_shared<ProtocolRequestPacket>(42)});

  shared_ptr<ProtocolRequestPacket> received;
  ASSERT_TRUE(waitUntil([&]() {
    client.receive();
    if (auto packet = client.pullSingle())
      received = as<ProtocolRequestPacket>(packet);
    return (bool)received;
  }));

  EXPECT_EQ(42, received->requestProtocolVersion);
  EXPECT_GT(server.workerStats()[0].wakeups, beforeWakeups);
}

TEST(UniverseConnectionServer, EagerSendStatsRecordCallerThreadWrite) {
  UniverseConnectionServer server([](UniverseConnectionServer*, ConnectionId, List<PacketPtr>) {}, 1);
  EXPECT_FALSE(server.queueOnlySends());

  auto pair = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pair.first)));
  UniverseConnection client(std::move(pair.second));

  auto beforeStats = server.workerStats();
  server.sendPackets(1, {make_shared<ProtocolRequestPacket>(77)});

  shared_ptr<ProtocolRequestPacket> received;
  ASSERT_TRUE(waitUntil([&]() {
    client.receive();
    if (auto packet = client.pullSingle())
      received = as<ProtocolRequestPacket>(packet);
    return (bool)received;
  }));

  auto afterStats = server.workerStats();
  EXPECT_EQ(77, received->requestProtocolVersion);
  EXPECT_EQ(totalQueuedSendBatches(beforeStats) + 1, totalQueuedSendBatches(afterStats));
  EXPECT_EQ(totalEagerSendBatches(beforeStats) + 1, totalEagerSendBatches(afterStats));
  EXPECT_EQ(totalEagerSendPackets(beforeStats) + 1, totalEagerSendPackets(afterStats));
  EXPECT_EQ(totalWorkerSendBatches(beforeStats), totalWorkerSendBatches(afterStats));
}

TEST(UniverseConnectionServer, QueueOnlySendIsOwnedByWorker) {
  UniverseConnectionServer server([](UniverseConnectionServer*, ConnectionId, List<PacketPtr>) {}, 1, true);
  EXPECT_TRUE(server.queueOnlySends());

  auto pair = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pair.first)));
  UniverseConnection client(std::move(pair.second));

  auto beforeStats = server.workerStats();
  server.sendPackets(1, {make_shared<ProtocolRequestPacket>(88)});

  shared_ptr<ProtocolRequestPacket> received;
  ASSERT_TRUE(waitUntil([&]() {
    client.receive();
    if (auto packet = client.pullSingle())
      received = as<ProtocolRequestPacket>(packet);
    return (bool)received;
  }));

  auto afterStats = server.workerStats();
  EXPECT_EQ(88, received->requestProtocolVersion);
  EXPECT_EQ(totalQueuedSendBatches(beforeStats) + 1, totalQueuedSendBatches(afterStats));
  EXPECT_EQ(totalEagerSendBatches(beforeStats), totalEagerSendBatches(afterStats));
  EXPECT_EQ(totalWorkerSendBatches(beforeStats) + 1, totalWorkerSendBatches(afterStats));
  EXPECT_EQ(totalWorkerSendPackets(beforeStats) + 1, totalWorkerSendPackets(afterStats));
}

TEST(UniverseConnectionServer, ManyIdleConnectionsRemainSharded) {
  UniverseConnectionServer server([](UniverseConnectionServer* server, ConnectionId clientId, List<PacketPtr> packets) {
      server->sendPackets(clientId, std::move(packets));
    }, 4);

  List<UniverseConnection> clients;
  for (ConnectionId clientId = 1; clientId <= 32; ++clientId) {
    auto pair = LocalPacketSocket::openPair();
    server.addConnection(clientId, UniverseConnection(std::move(pair.first)));
    clients.append(UniverseConnection(std::move(pair.second)));
  }

  ASSERT_TRUE(waitUntil([&server]() { return totalOwnedConnections(server.workerStats()) == 32; }));
  auto stats = server.workerStats();
  ASSERT_EQ(4, stats.size());
  for (auto const& workerStats : stats)
    EXPECT_EQ(8, workerStats.ownedConnections);

  ASSERT_TRUE(waitUntil([&server]() { return totalConnectionScans(server.workerStats()) >= 32; }));
  ASSERT_TRUE(waitUntil([&server]() { return totalIdleTimedWaits(server.workerStats()) > 0; }));

  auto activeClientId = ConnectionId(17);
  auto& activeClient = clients[activeClientId - 1];
  auto beforePackets = totalPacketsProcessed(server.workerStats());
  activeClient.pushSingle(make_shared<ProtocolRequestPacket>(1234));
  EXPECT_TRUE(activeClient.sendAll(SyncWaitMillis));

  shared_ptr<ProtocolRequestPacket> echoedPacket;
  ASSERT_TRUE(waitUntil([&]() {
    activeClient.receive();
    if (auto packet = activeClient.pullSingle())
      echoedPacket = as<ProtocolRequestPacket>(packet);
    return (bool)echoedPacket;
  }));
  EXPECT_EQ(1234, echoedPacket->requestProtocolVersion);
  EXPECT_GT(totalPacketsProcessed(server.workerStats()), beforePackets);

  auto beforeWakeups = totalWakeups(server.workerStats());
  server.sendPackets(activeClientId, {make_shared<ProtocolRequestPacket>(4321)});
  shared_ptr<ProtocolRequestPacket> serverPacket;
  ASSERT_TRUE(waitUntil([&]() {
    activeClient.receive();
    if (auto packet = activeClient.pullSingle())
      serverPacket = as<ProtocolRequestPacket>(packet);
    return (bool)serverPacket;
  }));
  EXPECT_EQ(4321, serverPacket->requestProtocolVersion);
  EXPECT_GT(totalWakeups(server.workerStats()), beforeWakeups);
  EXPECT_EQ(32, totalOwnedConnections(server.workerStats()));
}

TEST(UniverseConnectionServer, PacketOrderingAcrossWorkers) {
  UniverseConnectionServer server([](UniverseConnectionServer* server, ConnectionId clientId, List<PacketPtr> packets) {
      server->sendPackets(clientId, std::move(packets));
    }, 2);

  auto pairA = LocalPacketSocket::openPair();
  auto pairB = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pairA.first)));
  server.addConnection(2, UniverseConnection(std::move(pairB.first)));
  UniverseConnection clientA(std::move(pairA.second));
  UniverseConnection clientB(std::move(pairB.second));

  unsigned const BurstCount = 32;
  for (unsigned i = 0; i < BurstCount; ++i) {
    clientA.pushSingle(make_shared<ProtocolRequestPacket>(i));
    clientB.pushSingle(make_shared<ProtocolRequestPacket>(1000 + i));
  }
  EXPECT_TRUE(clientA.sendAll(SyncWaitMillis));
  EXPECT_TRUE(clientB.sendAll(SyncWaitMillis));

  List<unsigned> receivedA;
  List<unsigned> receivedB;
  ASSERT_TRUE(waitUntil([&]() {
    clientA.receive();
    while (auto packet = clientA.pullSingle())
      receivedA.append(convert<ProtocolRequestPacket>(packet)->requestProtocolVersion);

    clientB.receive();
    while (auto packet = clientB.pullSingle())
      receivedB.append(convert<ProtocolRequestPacket>(packet)->requestProtocolVersion);

    return receivedA.size() == BurstCount && receivedB.size() == BurstCount;
  }));

  for (unsigned i = 0; i < BurstCount; ++i) {
    EXPECT_EQ(i, receivedA[i]);
    EXPECT_EQ(1000 + i, receivedB[i]);
  }
}

TEST(UniverseConnectionServer, QueueOnlySendPreservesPacketOrdering) {
  UniverseConnectionServer server([](UniverseConnectionServer* server, ConnectionId clientId, List<PacketPtr> packets) {
      server->sendPackets(clientId, std::move(packets));
    }, 1, true);

  auto pair = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pair.first)));
  UniverseConnection client(std::move(pair.second));

  unsigned const BurstCount = 32;
  for (unsigned i = 0; i < BurstCount; ++i)
    client.pushSingle(make_shared<ProtocolRequestPacket>(i));
  EXPECT_TRUE(client.sendAll(SyncWaitMillis));

  List<unsigned> received;
  ASSERT_TRUE(waitUntil([&]() {
    client.receive();
    while (auto packet = client.pullSingle())
      received.append(convert<ProtocolRequestPacket>(packet)->requestProtocolVersion);

    return received.size() == BurstCount;
  }));

  for (unsigned i = 0; i < BurstCount; ++i)
    EXPECT_EQ(i, received[i]);

  auto stats = server.workerStats();
  EXPECT_EQ(0, totalEagerSendBatches(stats));
  EXPECT_GT(totalWorkerSendBatches(stats), 0);
}

TEST(UniverseConnectionServer, RemoveConnectionDuringCallback) {
  atomic<bool> callbackRan(false);
  UniverseConnectionServer server([&callbackRan](UniverseConnectionServer* server, ConnectionId clientId, List<PacketPtr>) {
      auto removed = server->removeConnection(clientId);
      callbackRan = true;
    }, 1);

  auto pair = LocalPacketSocket::openPair();
  server.addConnection(1, UniverseConnection(std::move(pair.first)));
  UniverseConnection client(std::move(pair.second));

  client.pushSingle(make_shared<ProtocolRequestPacket>(7));
  client.send();

  ASSERT_TRUE(waitUntil([&callbackRan]() { return callbackRan.load(); }));
  EXPECT_FALSE(server.hasConnection(1));
  EXPECT_EQ(0, totalOwnedConnections(server.workerStats()));
}
