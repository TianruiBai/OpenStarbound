...
- Phase 2 is implemented and live behind its fallback flag. Focused tests cover local and remote state-machine success, remote OpenStarbound/legacy TCP success, protocol mismatch rejection, password success/failure, asset mismatch, max-player/admin priority, duplicate UUID admin/non-admin priority, protocol-request timeout, client-connect timeout, password-response timeout, and slow login bursts, while `/serverstatus` exposes pending handshakes by state plus accepted/finalized/rejected/timed-out counters. Broader soak and high-connection profiling remain hardening before removing the legacy accept-thread fallback.
- Phase 3 has its compatibility-first persistence slice in place and default-enabled in the OpenStarbound game config: immutable universe/client/system snapshots, bounded async JSON persistence, synchronous fallback on queue pressure, retry accounting, shutdown draining, and server diagnostics. Focused tests now cover async triggered-storage completion, queue-full synchronous fallback, readable saved files, shutdown draining of queued writes, reload of written settings, and deterministic write-failure/retry accounting; broader latency and old-save validation remain required before removing fallback paths or widening the async write scope.
...
- Phase 5 has its compatibility-first snapshot/prep slice in place: client-owned/slave entity ids are indexed by owner connection, `WorldServer::update()` builds a per-tick `WorldTickSnapshot` with client windows and monitoring regions, liquid/weather/sector signaling/packet prep consume that snapshot, world generation sorting memoizes nearest-player sector distances, and packet prep reuses sector tile-array packets plus immutable entity-create type/store payloads within a tick. Blank entity-update deltas are still delivered to every indexed entity for that owner, preserving interpolation/extrapolation behavior; first net-state writes remain per-client because they advance entity net versions. Focused Phase 6 equivalence tests now compare tile-array packet payloads and controlled entity create/update packet payloads against the serial path, the two implemented Phase 6 worker helpers run default-enabled serial-versus-worker differential checks with divergence counters, and serial liquid/falling-block/wiring/entity/Lua baseline counters plus explicit mutation gate flags are available before any parallel mutation path lands. Deeper immutable net-state snapshots are still a Phase 6 gate before workerized entity packet prep.
...
- Asset/mod loading already has a worker queue for normal asset loads and post-process work, but source registration, patch ordering, `onLoad`/`postLoad` Lua scripts, and Lua patch contexts are order-sensitive. Future loading parallelism should parallelize source path enumeration and independent asset decode/parse work only behind deterministic source-order merging; Lua load scripts and patch application order should remain serial unless a mod explicitly opts into a new compatibility contract.
...
8. Done for focused coverage: the extended local/TCP/legacy/OpenStarbound/password/timeout/duplicate UUID matrix passes; the old accept-thread fallback remains for broader soak and high-connection profiling.
...
- Covered by focused tests: local state-machine success, remote OpenStarbound and legacy state-machine success, remote OpenStarbound and legacy TCP success, protocol mismatch rejection, password success/failure, duplicate UUID as admin and non-admin, login bursts with deliberately slow clients, asset mismatch refusal, max-player refusal with administrator priority, protocol-request timeout, client-connect timeout, and password-response timeout.
- Remaining hardening before removing the fallback: broader high-connection profiling, asset-mismatch refusal on both server/client opt-in sides, and long-running TCP soak across legacy/OpenStarbound clients.
...
1. `UniverseServer::doTriggeredStorage()` saves `universe.dat`, saves `tempworlds`, reads each active ship world through `WorldServerThread::readChunkUpdate()` when a client-context baseline exists, serializes `ServerClientContext::storeServerData()`, writes each `.clientcontext`, and then calls `CelestialMasterDatabase::cleanupAndCommit()`.
...
- `ServerClientContext::ShipChunksSnapshot` captures immutable full-chunk and delta data at the `readChunks()` boundary for full-snapshot tests, while `ServerClientContext::applyShipChunksUpdate()` applies incremental chunk updates directly into server-held context state and the pending client update stream.
...
2. Started gate before workerized packet prep: `WorldTickSnapshot` now caches immutable entity-create type/store inputs for shared create-packet preparation; immutable monitored entity ids, client net rules, pending tile/liquid/damage update copies, and net-state inputs are still required before worker jobs may own entity packet prep.
3. Started gate before workerized packet prep: controlled serial-versus-prefill entity create/update packet payload coverage exists for deterministic item-drop traffic. Deeper coverage is still needed around `writeNetState(0)` call counts and later delta behavior before net-state packet prep can move to workers. Tile-array sector prefill has focused serial-versus-prefill packet payload coverage.
...
3. Done: sector packet and entity store cache metrics are live; first-update serialization remains per-client until deeper packet-equivalence tests prove `writeNetState(0)` call-count changes are safe.
4. Phase 6 gate: move packet preparation into a post-tick job only after the remaining immutable entity net-state inputs and deeper entity packet-equivalence tests exist; tile-array sector prefill and controlled entity create/update packet payloads now have serial-equivalence guards.
...
1. Entity packet preparation: immutable entity-create type/store inputs and controlled byte-equivalence tests for entity create/update packet payloads now exist. Remaining work before expanding workerized packet preparation beyond tile-array sector prefill is immutable net-state inputs plus deeper `writeNetState(0)` and later-delta equivalence coverage, because `Entity::writeNetState(0)` still advances per-entity net versions.
...
- `phase6WorldParallelism.*MutationParallelism` flags are present but default-disabled, and requesting them records fixed-seed, dependency-analysis, mod-visibility, implementation, and worker-subsystem gate status. When every explicit gate is satisfied, mutation workers run immutable signature jobs only; live subsystem mutation remains serial.
- `phase6WorldParallelism.mutationParallelismFixedSeedSignatures` now enables serial fixed-seed signature preflight for liquid, falling blocks, wiring, entity, and Lua. In that opt-in mode, liquid and falling-block random sources use deterministic preflight seeds, liquid records active-cell and region signatures, falling blocks record pending/processed/moved/next-pending signatures, wiring records aggregate topology/output signatures, and entity/Lua record scalar post-serial work signatures. This remains diagnostics only and does not enable live mutation workers.
- Focused regression coverage verifies the implemented Phase 6 flags are enabled by default in the game config, that explicit opt-out still works, that the enabled paths record planning/prefill work without changing packet output, that differential checks run without divergences for the controlled cases, that serial mutation baseline metrics are stable across repeated controlled runs, that packet-sector prefill emits the same tile-array update payloads as the serial sector-packet path for a controlled two-client world, that controlled entity packet payloads remain equivalent, and that requested mutation parallelism stays blocked behind explicit gates until immutable signature workers are eligible.
...
- config surface that can disable each experiment independently without changing save or packet formats; storage generation planning, packet-sector prefill, differential checks, and subsystem baseline metrics are default-enabled in the OpenStarbound game config with explicit opt-out flags, while mutation-parallelism flags are default-disabled and expose gate status rather than enabling workers
...
- Live liquid, falling, wiring, entity, and Lua mutation requests remain blocked until fixed-seed signatures, dependency analysis, mod-visibility contracts, and a concrete implementation gate are all satisfied; immutable shadow-worker signature jobs may run after all explicit gates pass.
...
6. Mutation experiment graduation: graduate liquid, falling-block, and wiring experiments one subsystem at a time only after fixed-seed signatures, dependency analysis, mod-visibility contracts, implementation gates, differential checks, and fallback behavior are all in place. The first opt-in fixed-seed signature preflight exists for those three subsystems; it is diagnostic only and is not sufficient by itself to enable mutation workers.
...
3. Phase 2 pending handshakes are implemented behind `usePendingConnectionStateMachine` with focused extended-matrix coverage; the legacy accept-thread fallback is retained for broader soak and high-connection profiling.
4. Phase 3 immutable JSON persistence snapshots, bounded async executor, synchronous fallback, retry accounting, shutdown draining, focused queue-pressure/shutdown-drain/reload/write-failure tests are implemented while async persistence is default-enabled in the game config with explicit opt-out.
...
6. Phase 5 serial snapshot/prep groundwork is implemented: owner-indexed entity updates, per-tick monitoring-region snapshot reuse, generation-priority memoization, sector packet cache, immutable entity-create store cache, packet-prep counters, and focused regression tests.
7. Phase 6 storage-generation planning and packet-sector prefill are default-enabled in the game config with serial fallback diagnostics, default-enabled differential checks, divergence counters, default-enabled serial liquid/falling-block/wiring/entity/Lua baseline metrics, focused guard/equivalence tests, controlled entity packet-equivalence coverage, and explicit mutation gate status; deeper liquid, falling-block, wiring, entity, and Lua parallel mutation is intentionally deferred.
...
2. Soak the extended pending-handshake matrix under broader high-connection TCP load before removing the legacy accept-thread fallback; focused coverage now includes password, duplicate UUID, asset mismatch, max-player/admin-priority, login bursts, legacy/OpenStarbound TCP paths, and password/challenge flush timeouts.
3. Validate async persistence under deeper save/load depth, disconnect/ship-upgrade while save is pending, old-save loading, and latency profiling before removing synchronous fallbacks or broadening async persistence scope; focused reload and write-failure/retry tests now exist.
...
5. Extend entity packet-equivalence tests and immutable entity serialization inputs before moving packet preparation onto worker jobs; sector tile-array packet prefill and controlled entity create/update payloads now have focused serial-equivalence coverage, while immutable net-state inputs remain future work.
6. Keep Phase 6 experiments isolated, independently disable-able, and backed by differential tests; the first differential-check path now covers the two implemented worker helpers, liquid/falling/wiring/entity/Lua now have serial baseline counters, and mutation-parallelism requests are explicitly blocked by fixed-seed, dependency-analysis, mod-visibility, and implementation gates until a compatible API contract lands.
