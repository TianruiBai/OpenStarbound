...
- `phase6WorldParallelism.*MutationParallelism` flags are present but default-disabled, and requesting them records fixed-seed, dependency-analysis, mod-visibility, and implementation gate status instead of enabling mutation workers.
- `phase6WorldParallelism.mutationParallelismFixedSeedSignatures` now enables the first serial fixed-seed signature preflight for liquid, falling blocks, and wiring. In that opt-in mode, liquid and falling-block random sources use deterministic preflight seeds, liquid records active-cell and region signatures, falling blocks record pending/processed/moved/next-pending signatures, and wiring records aggregate topology/output signatures. This remains diagnostics only and does not enable mutation workers.
- Focused regression coverage verifies the implemented Phase 6 flags are enabled by default in the game config, that explicit opt-out still works, that the enabled paths record planning/prefill work without changing packet output, that differential checks run without divergences for the controlled cases, that serial mutation baseline metrics are stable across repeated controlled runs, that packet-sector prefill emits the same tile-array update payloads as the serial sector-packet path for a controlled two-client world, that controlled entity packet payloads remain equivalent, and that requested mutation parallelism stays blocked behind the explicit gates.
...
- Parallel liquid, falling, wiring, entity, and Lua mutation requests remain blocked until fixed-seed signatures, dependency analysis, mod-visibility contracts, and a concrete implementation gate are all satisfied.
